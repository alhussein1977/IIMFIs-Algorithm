...
public class MFITree {
	 ...
	 */
	public void addMFI(Integer[] itemset, int itemsetLength, int support) {
		
		MFINode currentNode = root;
		// For each item in the itemset
		for(int i=0; i < itemsetLength; i++){
			Integer item = itemset[i];
...
//		currentNode.counter = support;
	...
	private boolean issASubsetOfPrefixPath(List<Integer> headWithP,	MFINode node) {
		...
		if(node.level >= headWithP.size()) {
			...
			Integer itemToLookFor = headWithP.get(positionInItemset);
			// for each item in itemset
			do {
				if(nodeToCheck.itemID == null ? itemToLookFor == null : nodeToCheck.itemID.equals(itemToLookFor)) {
					positionInItemset--;
